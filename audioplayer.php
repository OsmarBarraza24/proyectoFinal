...
if(isset($_GET["playlist"])){
    $queryObtenerPlaylistCancionAlbumArtista = "SELECT playlist.id, playlist.nombre, cancion.id, cancion.nombre, cancion.ruta
...
<div style="margin-top:5rem;">
    ...
    <div id="amazingaudioplayer-1" style="display:block;position:relative;width:100%;height:auto;margin:0px auto 0px;">
        ...
        <?php while($dataPlaylistCancion = mysql_fetch_array($resQueryObtenerPlaylistCancion)){
            $queryObtenerAlbum = "SELECT album.id, album.nombre, album.imagen
            ...
            $dataArtista = mysql_fetch_array($resQueryObtenerArtista);
        ?>
            <li data-artist=<?php echo '"'. $dataArtista[1] .'"'?> data-title=<?php echo '"'. $dataPlaylistCancion[3] .'"'?> data-album=<?php echo '"'. $dataAlbum[1] . '"'?> data-info="" data-image=<?php echo '"imagenes/'. $dataAlbum[2] .'"'?>>
                <div class="amazingaudioplayer-source" data-src=<?php echo '"audios/'. $dataPlaylistCancion[4] .'"'?> data-type="audio/mpeg"></div>
            </li>
        <?php }?>
        </ul>
